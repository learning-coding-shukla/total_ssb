...
function PPDT() {
  ...
  useEffect(() => {
    ...
    if (phase === "character" && characterTime > 0) {
      const timer = setTimeout(() => {
  ...
  const nextImage = () => {
    const attempts =
      JSON.parse(localStorage.getItem("ppdt_attempts_data")) || [];
    ...
    localStorage.setItem("ppdt_attempts_data", JSON.stringify(attempts));
  ...
  const isCompleted = currentIndex >= ppdtImages.length;
    ...
    return (
      <div className="max-w-6xl mx-auto p-10 text-center">
        <div className="bg-gradient-to-r from-slate-900 via-green-950 to-slate-900 rounded-3xl p-12 text-white border border-yellow-500/30">
          <h1 className="text-6xl font-bold mb-6">🏅 PPDT Session Completed</h1>
          ...
          <p className="mt-4 text-slate-400">
            Continue developing observation, leadership and storytelling skills.
          </p>
        </div>
      </div>
    );
  ...
  console.log("Images:", ppdtImages);
  console.log("Length:", ppdtImages.length);
  ...
  return (
    <div className="max-w-6xl mx-auto p-10">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-green-950 to-slate-900 rounded-3xl p-8 mb-10 text-white border border-yellow-500/30">
        <h1 className="text-5xl font-bold">🖼️ PPDT Practice</h1>

        <p className="mt-3 text-slate-300">Observe • Analyse • Lead</p>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden mt-4">
          <div
            ...
            style={{
              width: `${((currentIndex + 1) / ppdtImages.length) * 100}%`,
            }}
          />
        </div>
      </div>
      ...
      <div className="text-center mb-10">
        ...
        <p className="mt-4 text-xl font-semibold">
          {phase === "observation" && "🖼 Observe the Picture"}

          {phase === "character" && "👤 Character Identification"}

          {phase === "story" && "✍ Story Writing"}
        </p>
      </div>
      ...
      {phase === "character" && (
        <div className="bg-blue-50 border rounded-2xl p-8 max-w-3xl mx-auto">
          ...
          <p className="mb-6">Identify the main character.</p>
      ...
      {phase === "story" && (
        <div className="bg-green-50 border rounded-2xl p-8 max-w-3xl mx-auto">
          <h2 className="text-3xl font-bold mb-4">✍ Story Writing Phase</h2>

          <p className="mb-6">Write your story in your notebook.</p>
          ...
          {storyTime === 0 && (
            <div className="mt-8 p-5 bg-red-100 rounded-xl">
              <h3 className="text-red-600 text-2xl font-bold">⏰ Time Over</h3>

              <p className="mt-2">
                Review your notebook and move to the next picture.
              </p>
            </div>
          )}
        </div>
